...
<Columns cols={2}>
  <Card title="System Context" icon="diagram-project" href="/architecture/system-context">
    C4 C1 view of users, external systems, the privacy pool, and the auditing portal.
  </Card>
  <Card title="Architecture Map" icon="network-wired" href="/architecture/architecture-map">
    C4 C2 view of deployable/runtime containers and the event path between them.
  </Card>
...
flowchart LR
  ...
  scanner["ScannerModule and Stellar RPC client"]
  audit["audit"]
  interpretation["audit_interpretation"]
  workflow["Cases, reports, auditors_log"]
  ui["Audit UI"]
