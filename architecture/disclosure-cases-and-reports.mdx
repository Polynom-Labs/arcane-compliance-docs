...
- `event_type`, `user`, `org_id`, external identity user reference
- Typed `object` and `details` JSONB (with `type` and `version`)
- Optional application and case references
...
| Application | `GET .../applications/:applicationId/auditors-log` | Common access — View activity log (`logs:view_activity`) |
