...
flowchart LR
  ...
  subgraph sdkParts["SDK responsibilities"]
    stealth["Stealth address helpers"]
    coin["Coin construction"]
    witness["Merkle witness preparation"]
    proof["Groth16 proof generation"]
    serialization["Soroban serialization"]
  end

  subgraph contract["PrivacyPoolsContract"]
    entry["transact(from, proof_bytes, pub_signals_bytes, encoded)"]
    verifier["Groth16 verifier"]
    roots["Root history"]
    nullifiers["Consumed nullifier hashes"]
    commitments["Commitment tree and leaf ephemeral keys"]
    tokens["Public deposit and withdrawal token transfers"]
